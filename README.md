...
## Validate
`npm run lint && npm run typecheck && npm test && npm run build && npm run test:e2e`. During the pre-revenue stage these checks run locally and inside Codex before each pull request; the repository intentionally does not use paid GitHub Actions minutes. Supabase migrations define production schema/RLS; configure secrets from `.env.example`, never in Git.
